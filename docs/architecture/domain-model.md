...
| --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Task fields or completion status        | `services/todos/edits.ts`: `applyTodoEdit`; `server/controllers/todos/controller.ts`: `update`; `server/services/todos/catalog.ts`: `getForEdit`, `update`                                                           | `server/controllers/todos/edit-rules.spec.ts`; `server/controllers/todos/complete-edit.spec.ts`                                                      |
| Folder placement or sibling order       | `server/controllers/projects/controller.ts`: `move`; `server/services/projects/catalog.ts`: `readForMove`, `decideProjectEntryMove`, `persistOrder`; `services/notes/creation.ts`: `decideNoteCreation`              | `server/controllers/projects/placement.spec.ts`; `controllers/workspace/command-creation.spec.ts`                                                    |
| Archive or restore a note               | `services/notes/trash.ts`: `noteTrashChange`; `stores/workspace/resources.svelte.ts`: `prepareCommand`                                                                                                               | `server/controllers/notes/restore.spec.ts`; `controllers/workspace/command-trash.spec.ts`                                                            |
| Save a document or publish a snapshot   | `server/controllers/notes/controller.ts`: `save`, `publish`, `discardDraft`, `restoreRevision`; `server/services/notes/catalog.ts`: `getForEdit`, `persistEdit`, `getForPublication`, `persistPublication`, `record` | `server/controllers/notes/save.spec.ts`; `server/controllers/notes/restore-revision.spec.ts`                                                         |
...
resolved entry records. Projects owns the move transaction, obtains the locked project tree from
ProjectCatalog and applies decideProjectEntryMove before persisting the resolved sibling order.
The project model contains values, types and schemas only.
