...
| Finish or cancel a durable run          | `server/controllers/agent/controller.ts`; `server/controllers/workflow-execution/controller.ts`; `server/services/agent/runs/settlement.ts`: `claim`, `complete`                       | `server/services/agent/runs/settlement.spec.ts`; `server/controllers/agent/lifecycle.spec.ts`; `tests/integration/agent/settlement.contract.spec.ts` |
