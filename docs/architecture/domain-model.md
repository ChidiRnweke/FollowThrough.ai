...
| Folder placement or sibling order       | `server/services/projects/catalog.ts`: `createFolder`, `move`; `models/workspace-mutations/index.ts`: `newNote`                                              | `server/services/projects/catalog.spec.ts`; `models/workspace-mutations/creation.spec.ts`                      |
| Archive or restore a note               | `server/services/notes/catalog.ts`: `archive`, `restore`; `models/workspace-mutations/index.ts`: `noteTrashWrite`                                            | `server/controllers/notes/restore.spec.ts`; `models/workspace-mutations/trash.spec.ts`                         |
...
| Skill metadata or imported instructions | `server/controllers/skills/controller.ts`: `update`, `restoreVersion`; `server/services/skills/library.ts`: `update`, `restoreVersion`                       | `server/services/skills/library.spec.ts`; `server/controllers/skills/create.spec.ts`                           |
| Trace a proposal back to selected text  | `server/services/notes/catalog.ts`: `createAnchor`; `server/services/notes/provenance.ts`: `record`; the reference, relationship, todo and skill controllers | `server/controllers/todos/extract-promises.spec.ts`; `server/controllers/skills/create-from-selection.spec.ts` |
| Accept or undo an agent proposal        | `server/controllers/suggestions/controller.ts`: `accept`, `revert`; `server/services/suggestions/application.ts`: `apply`, `revert`                          | `server/controllers/suggestions/lifecycle.spec.ts`; `server/services/suggestions/application.spec.ts`          |
...
Several rows currently name both browser and server implementations. Both need inspection until
the shared-rule extraction lands. The work and measurements are tracked in
[the composition checklist](../plans/domain-composition.md).
