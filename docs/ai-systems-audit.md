...
A fast, cheap model predicts the next span as **ghost text**; `Tab` accepts, `Esc` dismisses — suggestions appear without the user asking. Very feasible: the editor is real Tiptap/ProseMirror, `edra/AI.svelte:204` already does positioned streaming inserts, and `note-editor.svelte:250` registers runtime plugins, so a ghost-text decoration plugin slots in the same way. What makes it *smart*: condition completions on retrieved project context (§2.2), so it completes with the user's own notes/terminology. Debounce, fire on pause, cancel in-flight on keystroke; ghost text is a decoration (never real content until accepted) so it never touches undo history.
