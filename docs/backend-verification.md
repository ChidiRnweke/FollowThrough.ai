...
| Controllers are domain nouns and orchestrate injected capabilities | Domain folders under `src/lib/controllers`; `ControllerFactory` exposes workspace, projects, notes, todos, relationships, references, diagrams, suggestions, skills, agent settings, agent, and trust policies. |
...
| Skills are advertised by summary, loaded lazily with usage provenance, provisioned idempotently, and versioned       | Skill management/provisioning and agent-context behavior suites plus live skill-usage contract                | Covered |
...
| Agent context includes project, note, selection, retrieval, conversation history, and lazy skill summaries           | Agent controller and enriched-context behaviors                                                               | Covered |
| Conversation overrides, effective models, messages, and tool activity persist chronologically and remain actor-owned | Agent controller behaviors and live conversation/agent-settings contracts                                     | Covered |
| Tool coverage is compile-time exhaustive; tools reuse controllers and preserve proposal review                       | Agent tool-registry classification and approval behavior suites                                               | Covered |
| Mutation approval and auto-accept are explicit; paused run state and pending decisions remain actor-owned            | Agent tool-registry behavior and live agent-run repository contracts                                          | Covered |
| OpenRouter model precedence, tool support, cache, and stale fallback are deterministic                               | Agent settings behavior suite                                                                                 | Covered |
