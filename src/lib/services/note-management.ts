...
import { NotFoundError, OwnershipError, StaleRevisionError, ValidationError } from '../models';
import type { NoteRepository, ProjectRepository, SourceAnchorRepository } from '../repositories';
import type {
	NoteCreator,
	NoteEditor,
	NoteReader,
		...
		SourceAnchorRepairer
{
	...
	async save(actor: ActorContext, candidate: Note): Promise<Note> {
		...
		if (candidate.parentId) await this.validateParent(actor, current, candidate.parentId);
	...
	async record(
		actor: ActorContext,
		note: Note,
		provenance?: import('../models').Provenance
	): Promise<void> {
