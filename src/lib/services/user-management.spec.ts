...
describe('User boundary invariants', () => {
	it('creates the local actor record when it is absent', async () => {
		...
		const user = await new UserManagementService(repository).get(testActor());
	...
	it('returns not-found when persistence cannot establish the actor', async () => {
		...
		await expect(new UserManagementService(repository).get(testActor())).rejects.toMatchObject({
			code: 'NOT_FOUND'
		});
	});
});
