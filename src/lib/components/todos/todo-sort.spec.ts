...
describe('Todo table sorting', () => {
	...
	it('undated todos sort last ascending', () => {
		const views = [
			view({ title: 'none' }),
			view({ title: 'dated', dueDate: '2026-08-02' as never })
		];
	...
	it('project sorts by resolved name with unknown projects last', () => {
		...
		const views = [view({ title: 'unknown' }), view({ title: 'known', projectId: known })];
	...
	it('source sorts by the source note title', () => {
		const views = [view({ title: 'no source' }), view({ title: 'sourced' }, 'Alpha note')];
