...
import { DefaultRetrievalController } from './controller';
...
describe('DefaultRetrievalController', () => {
	...
	it('trims result content to an excerpt', async () => {
		...
		const results = await new DefaultRetrievalController({
			knowledgeSearcher: searcher,
			condenser,
			conversations: journalOf([])
