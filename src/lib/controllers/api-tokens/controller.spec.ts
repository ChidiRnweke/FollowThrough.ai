...
import type {
	...
	MintedApiToken,
	VerifiedApiToken
} from '$lib/services/auth/apiTokenService';
import { testActor, testNow } from '$lib/testing/fixtures/domain-builders';
import { DefaultApiTokensController } from './controller';
...
class FakeApiTokenService implements IApiTokenService {
...
describe('API token controller behavior', () => {
	it('lists credentials owned by the actor', async () => {
		const tokens = new FakeApiTokenService();
		const controller = new DefaultApiTokensController({ tokens });
