...
import type { KnowledgeSearcher, MemoryEntryLister, Reranker } from '$lib/services';
import {
	memoryEntryBuilder,
	noteBuilder,
	testActor,
...
const build = (
	...
	new RetrievalInlineCompletionContextBuilder({
...
describe('inline completion context', () => {
	...
	it('degrades project retrieval failure to empty passages', async () => {
		const context = await new RetrievalInlineCompletionContextBuilder({
