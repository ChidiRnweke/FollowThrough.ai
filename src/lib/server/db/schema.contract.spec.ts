...
describe('Postgres schema contracts', () => {
	...
	it('uses private user ownership rather than workspaces', async () => {
		const rows = await context.client<
			{ column_name: string }[]
		>`select column_name from information_schema.columns where table_name = 'notes' order by column_name`;
		expect(rows.map((row) => row.column_name)).toContain('user_id');
	});
	it('enforces case-insensitive entity uniqueness per user and type', async () => {
		const rows = await context.client<
			{ indexdef: string }[]
		>`select indexdef from pg_indexes where indexname = 'entities_user_type_name_unique'`;
