...
const setup = async () => {
	...
	const change = requirePreparedChange(
		await fixture.controller.prepareChange(testActor(), {
			kind: 'patch',
			noteId: note.id,
...
describe('Reviewed note changes', () => {
	...
	it('saves the prepared document', async () => {
		...
		await controller.applyReviewedChange(testActor(), change);
	...
	it('refuses to apply after a different edit', async () => {
		...
		expect(await controller.applyReviewedChange(testActor(), change)).toMatchObject({
			kind: 'failure',
			code: 'STALE_REVIEW'
		});
	});
	...
	it('keeps a failed anchor as a preparation failure', async () => {
		...
		expect(
			await controller.prepareChange(testActor(), {
				kind: 'patch',
				noteId: testNoteId(),
	...
	it('does not prepare another account’s note', async () => {
		const { controller } = reviewedNoteFixture(baseNote());
		await expect(
			controller.prepareChange(testActor(2), {
				kind: 'replace',
				noteId: testNoteId(),
				markdown: 'New'
			})
	...
	it('does not apply another account’s review', async () => {
		const { controller, change } = await setup();
		await expect(controller.applyReviewedChange(testActor(2), change)).rejects.toMatchObject({
			code: 'NOT_FOUND'
		});
	});
	it('refuses an archived note even if the result matches', async () => {
		...
		await expect(controller.applyReviewedChange(testActor(), change)).rejects.toMatchObject({
			code: 'VALIDATION'
		});
	});
	...
	it('rolls back note, links and indexing when a consequence fails', async () => {
		const { controller, content, note, change } = await setup();
		content.failIndex = true;
		const outcome = await controller.applyReviewedChange(testActor(), change).then(
			() => 'saved',
			() => 'failed'
		);
		expect({
